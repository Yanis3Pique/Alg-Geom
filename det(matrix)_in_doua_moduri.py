...
# Aici e dimensiunea maxima a matricei. Daca e prea mare, s-ar putea sa dureze foarte mult
y = int(input("Dati un numar de teste pe care vreti sa il efectuati: "))
for i in range(y):
    n = random.randint(1, x)  #Generează o valoare aleatorie pentru n între 1 și x
...
def test():
    x = int(input("Dați un număr natural între 1 și 100, inclusiv: "))
    # De preferat ar fi sa fie x <= 10, pentru ca altfel dureaza foarte mult compilarea.
    y = int(input("Dati un numar de teste pe care vreti sa il efectuati: "))
    # La y orice numar este bun. Eu am testat pentru 100, 500, 1000, 100000 si 1000000000 de exemplu.

    # memorăm timpul de execuție pentru fiecare dimensiune a matricei
    sizes = []
    t_trad = []
    t_gauss = []
    for i in range(1, x + 1):
        sizes.append(i)
        matrice = [[random.randint(1, 10) for j in range(i)] for k in range(i)]

        # timp pentru metoda tradițională
        timp_inceput_t = time.time()
        rez_t = traditional_det(matrice)
        timp_sfarsit_t = time.time()
        t_trad.append(timp_sfarsit_t - timp_inceput_t)

        # timp pentru metoda Gauss
        timp_inceput_g = time.time()
        rez_g = gaussian_det(matrice)
        timp_sfarsit_g = time.time()
        t_gauss.append(timp_sfarsit_g - timp_inceput_g)

    # afișăm un grafic
    plt.plot(sizes, t_trad, label='Tradițională')
    plt.plot(sizes, t_gauss, label='Gauss')
    plt.xlabel('Dimensiunea matricelor')
    plt.ylabel('Timpul de execuție (s)')
    plt.legend()
    plt.show()
